...
pub struct DensePolynomialVarGeneric<FP: FieldVar<F, CF>, F: Field, CF: PrimeField> {
    /// The coefficient of `x^i` is stored at location `i` in `self.coeffs`.
    pub coeffs: Vec<FP>,
    _params: PhantomData<(F, CF)>,
}

impl<FP: FieldVar<F, CF>, F: Field, CF: PrimeField> DensePolynomialVarGeneric<FP, F, CF> {
    /// Constructs a new polynomial from a list of coefficients.
